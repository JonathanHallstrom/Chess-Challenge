...
public class SidhantEvilBot : IChessBot
{
        ...
        foreach (bool side in new[] { false, true })
        {
            for (int piece_type = 1; piece_type <= 6; piece_type++)
            {
                ...
                while (bb > 0)
                {
                    ...
                    score_mg[Convert.ToInt32(side)] += pvm_mg[piece_type] + getPstVal(index);
                    score_eg[Convert.ToInt32(side)] += pvm_eg[piece_type] + getPstVal(index + 64);
